...
class Conv1dAdaIN(nn.Module):
    ...
    def forward(self, x, style):
        ...
        out = gamma * x + beta
        return out
#/home/humair/all_data/StyleTTS2/Utils

from all_data.StyleTTS2.Utils.ASR.models import ASRCNN
from all_data.StyleTTS2.Utils.JDC.model import JDCNet
from all_data.StyleTTS2.Modules.discriminators import MultiPeriodDiscriminator, MultiResSpecDiscriminator, WavLMDiscriminator
...
def build_model(args, text_aligner, pitch_extractor, bert):
    ...
    if args.decoder.type == "istftnet":
        from all_data.StyleTTS2.Modules.istftnet import Decoder
        ...
        from all_data.StyleTTS2.Modules.hifigan import Decoder
        decoder = Decoder(dim_in=args.hidden_dim, style_dim=args.style_dim, dim_out=args.n_mels,
                resblock_kernel_sizes = args.decoder.resblock_kernel_sizes,
                upsample_rates = args.decoder.upsample_rates,
                upsample_initial_channel=args.decoder.upsample_initial_channel,
...
class Args:
    n_token = 100  # example vocabulary size (adjust as per your model)
...
x = torch.randint(low=0, high=args.n_token, size=(batch_size, seq_len))

input_lengths = torch.tensor([50, 45])

m = torch.arange(seq_len).expand(batch_size, seq_len) < input_lengths.unsqueeze(1)
m = m.float()

text_encoded = TextEncoder(x, input_lengths, m)
print("TextEncoder output shape:", text_encoded.shape)
